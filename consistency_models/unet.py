...
blk_manage = lambda ic,oc: nn.Sequential(
    nn.GroupNorm(32, num_channels=ic),
    nn.ReLU(),
    nn.Conv2d(ic, oc, 3, padding=1),
    nn.GroupNorm(32, num_channels=oc),
...
def vne_loss(features):
    G = calculate_gram_matrix(features)
    vne = calculate_vne(G)
    vne_coeff = 0.1
    return -1*vne_coeff * vne
...
class ConsistencyModel(nn.Module):
    ...
    def loss_vne_inc(self, x, z, t1, t2, ema_model):
        x2 = x + z * t2[:, :, None, None]
        x2,feat = self.forward_feature(x2, t2)

        with torch.no_grad():
            x1 = x + z * t1[:, :, None, None]
            x1 = ema_model(x1, t1)

        return F.mse_loss(x1, x2) + vne_loss(feat)

    def loss_vne_diff(self, x, z, t1, t2, ema_model):
        x2 = x + z * t2[:, :, None, None]
        x2,feat = self.forward_feature(x2, t2)

        with torch.no_grad():
            x1 = x + z * t1[:, :, None, None]
            x1,feat1 = ema_model.forward_feature(x1, t1)

        feat1=feat1[torch.randperm(feat1.size()[0])]

        feat = feat.view(feat.size()[0],-1)
        feat1 = feat1.view(feat1.size()[0],-1)

        vne = calculate_vne(torch.abs(feat1-feat))

        return F.mse_loss(x1, x2) + vne
...
class ConsistencyModel_Easy(nn.Module):
    """
    This is ridiculous Unet structure, hey but it works!
    """

    def __init__(self, n_channel: int, eps: float = 0.002, D: int = 128) -> None:
        super(ConsistencyModel_Easy, self).__init__()
